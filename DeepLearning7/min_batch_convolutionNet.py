...
import SimpleConvNet as simpleConvNet
...
#读入数据  
train_images=Preprocessing.convolutiondo(Preprocessing.normalize(Minst.get_train_images()))
train_lables=Preprocessing.one_hot(Minst.get_train_lables())
test_images=Preprocessing.convolutiondo(Preprocessing.normalize(Minst.get_test_images()))
...
for i in range(iters_num):
    ...
    w2_list.append(np.mean(grad['W3']))
    b2_list.append(np.mean(grad['b3']))'''
...
plt.plot(x,w3_list,color='pink',label='w3_d')
plt.plot(x,b3_list,color='green',label='b3_d')'''
'''#参数情况
